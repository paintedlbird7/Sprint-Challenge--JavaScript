...
/* Request 1: Create a new array called universities that contains all the univeristies in the graduates 
//array.  
...
The zoo wants to display both the scientific name and the animal name in front of the habitats.  Return an 
//array with only the animal and scientific names in it.  The individual values in the array should look 
...
const lowerCase = [];
zooAnimals.map(item => toLowerCase.push(`${item.animal_name}`));
...
largerPopulation = zooAnimals.filter((id) => { return id.animal_size < 5;});
